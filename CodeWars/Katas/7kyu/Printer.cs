...
namespace CodeWars.Katas._7kyu
{
    ...
    /// which, for the sake of simplicity, are named with letters from a to m. The colors used by the printer
    /// are recorded in a control string. For example a "good" control string would be aaabbbbhaijjjm meaning
    ...
    /// is the number of errors and the denominator the length of the control string. Don't reduce this fraction to
    /// a simpler expression. The string has a length greater or equal to one and contains only letters from ato z.
    /// Examples:
    ///
    /// s="aaabbbbhaijjjm"
    ...
    public class Printer
    {
        public static string PrinterError(String s) =>
            $"{"abcdefghijklm".Count(x => !s.Contains(x)) + 1}/{s.Length}";
    }
}